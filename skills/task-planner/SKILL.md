...
Sketch 2-3 key screens. Include in `task-graph.md` Section 3 before the design tokens. The wireframes are throwaway — their purpose is to validate spatial decisions (sidebar vs. top nav, card layout, content hierarchy) before investing in pixel-level specs.
